...
bool fifo::full() {
	// std::cout << "Num elements: " << num_elements << endl;
	if (num_elements == size) return true;
...
// TODO: Implement req_r function
void fifo::req_r(bool req) {
	if(req) {
		if(!busy) {
			if(!empty()) {
				busy = true;
...
// TODO: Implement req_w function
void fifo::req_w(bool req) {
	if(req) {
		if(!busy) {
			if(!full()) {
				busy = true;
