...
inline void ea_pullup::update() {
  ...
  if (m_new_val == sc_dt::SC_LOGIC_Z) {
    m_new_val = sc_dt::SC_LOGIC_1;
  }//endif
  base_type::update();
}
...
inline void ea_pullup_pack<W>::update() {
  //sc_lv_resolve<const int>::resolve( this->m_new_val, m_val_vec );
  sc_lv_resolve<W>::resolve( this->m_new_val, m_val_vec );
  for (unsigned i=0;i!=W;i++) {
    if (m_new_val[i] == sc_dt::SC_LOGIC_Z) {
      m_new_val[i] = sc_dt::SC_LOGIC_1;
    }//endif
  }//endfor
  base_type::update();
}
