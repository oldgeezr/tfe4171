...
void gas_station::customer1_thread(void) 
{
  for (;;) 
  {
    ...
    cout << "INFO: " << name() << " Customer1 needs gas (1) at "
         << hms() << endl;
...
void gas_station::customer2_thread(void) 
{
  for (;;) 
  {
    ...
    cout << "INFO: " << name() << " Customer2 needs gas (1) at "
         << hms() << endl;
...
void gas_station::attendant_method(void) 
{
  ...
  if (!m_filling) 
  {
    ...
    {
      cout << "INFO: " << name() << " Filling tank1 (3) at " 
           << hms() << endl;
      ...
      {
        cout << "INFO: " << name() << " Filling tank2 (3) at " 
             << hms() << endl;
  ...
  if (m_tank1 == 0 && m_count1 < m_maxcount) 
  {
    m_tank1 = m_full1;
    m_count1++;
    cout << "INFO: " << name() << " Filled tank1 (4) at " 
         << hms() << endl;
  } else 
  if (m_tank2 == 0 && m_count2 < m_maxcount) 
  {
    m_tank2 = m_full2;
    m_count2++;
    cout << "INFO: " << name() << " Filled tank2 (4) at " 
         << hms() << endl;
  }//endif
    ...
    {
      cout << "WARN: " << name() << " No more fuel at " 
           << hms() << endl;
      sc_stop();
    }//endif
  }//endelse
...
{
  //cout << "hms sc_time_stamp()= " << sc_core::sc_time_stamp() << endl;
  std::ostringstream now;
  sc_core:: sc_time temp_time(sc_core::sc_time_stamp());
  temp_time = temp_time/(t_MIN.to_seconds());
  double mins(temp_time.to_seconds());
  // cout << "hms mins= " << mins << endl;
  unsigned days = int(mins/(24*60));
  mins -= days*24.0*60.0;
  unsigned hrs = int(mins/60);
  mins -= hrs*60.0;
  //unsigned mins = int(mins);
