...
void time_flies::time_thread(void) {
  ...
  for (t_DELAY=2*t_1HR;t_DELAY>=t_1FS;t_DELAY/=10) {
    cout << "INFO: Delaying " << setw(7) << t_DELAY << " ";
    wait(t_DELAY);
    cout << "INFO: Time now " << display_hms(sc_time_stamp()) << "!" << endl;
  }//endfor
  wait(SC_ZERO_TIME);
  cout << "INFO: wait(SC_ZERO_TIME)" << endl;
  cout << "INFO: Time still " << display_hms(sc_time_stamp()) << "!" << endl;
  #ifdef USE_STOP
...
std::string time_flies::display_hms(const sc_core::sc_time t) {
  // unsigned hours   = int(t / 3600.0); 
  double t_in_seconds   = t.to_seconds();
  unsigned hours        = int(t_in_seconds / 3600.0 );

  t_in_seconds -= 3600.0*hours;
  unsigned minutes = int(t_in_seconds / 60.0);
  t_in_seconds -= 60.0*minutes;
  double   seconds = t_in_seconds;
